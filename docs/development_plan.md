...
- 1.7.3 Multi-timeframe data pipeline (M1/M5/M15/H1/H4/D1/W1/MN) + consensus + microTrend. **Done 2026-06-01.**
- 1.7.4 Stateful guard counters (trap cooldown, shock freeze, deep reclaim, SMC trend state). **Done 2026-06-01.**
...
- 1.7.16 Pine V1 Gold deep-audit gap report — 8 divergences (BIAS/CONF analog vs vote-ratio regression, RECLAIM source bug, missing `altEWFactor` + daily-timeframe clamps, BIAS prefix, trap-type detector over-fire, missing SL pad components, missing `altTPFactor`, proxy `xauusd_bias` not propagating) catalogued against `docs/TV_Pine_Scripts/TV_GOLD_PAXG_XAUT_V1.pine.txt`. **Drafted 2026-06-05.** Commit 1 (Gap A + B + E — revert vote-ratio regression, fix RECLAIM source, add `Bias : ` prefix) **Done 2026-06-06.** Commit 2 (Gap C — `altEWFactor` per-asset EW compression via `AssetEvaluator::ew_compression_factor` trait method + 1D/sideways three-tier clamps in `EntryPlanCalculator`) **Done 2026-06-06.** Commit 3 (Gap G + H — `altSLFactor` plumbing via `AssetEvaluator::sl_extension_factor` + removed two non-Pine SL clamps; `altTPFactor` plumbing via `AssetEvaluator::tp_compression_factor` applied in `TakeProfits::from_context`) **Done 2026-06-06.** Commit 4 (Gap F+D proxy `xauusd_bias` plumbing + trap detector audit) pending. See `docs/phase1_pine_parity_plan.md` § 1.7.16.
- 1.7.17 Score composition Pine parity — full per-sub-layer accumulator rewrite of `evaluate_direction`. **Drafted 2026-06-08, Done 2026-06-09** (all 4 commits landed in one session). Commit 1: sub-layer accumulator skeleton — `WeightPair { pass, fail }` profile table, 17 sub-layer helpers (`bias_htf_4h`/`bias_htf_1d`/`trend_dir`/`trend_200`/`macd`/`rsi`/`dmi`/`adx_band`/`vwap`/`mtf_dominant`/`structure`/`smc_bonus`/`ltf_consensus`/`liquidity`/`support_resistance`/`volume`/`anti_trap`), 5 asset profiles migrated. Commit 2: penalty stack — `anti_chop`/`strong_conflict`/`macro_conflict`/`vol_shock` direction-aware helpers with Pine's −12/−18/−10/−4 weights. Commit 3: V61.6 micro-trend override (±14/±18) + V61.7 consensus weighting (+0.22 same dir, −0.18 opposite dir) applied as graded contributions outside the boolean `add` closure. Commit 4: V63 Gold direction adapter + V62 Altcoin analog via new `AssetEvaluator::score_adjustments` trait method (LTF edge weighting, gold proxy directional weight, reversal-OK bonus, macro-conflict relax, opposing-proxy penalty). 167/167 lib tests and 5/5 parity tests pass after each commit. BTC fixture `bias_text` migrated `Bias : MAP LONG 75% → 15% → 29% → 39%` across the 1.7.16/1.7.17 sequence. See `docs/phase1_pine_parity_plan.md` § 1.7.17 for the full implementation result.
- 1.7.18 Per-asset-class Pine-parity rewrite — extend the Gold rewrite template (1.7.16+1.7.17) to the remaining four asset classes. Four independent sub-phases A/B/C/D, each capturing a Pine reference screenshot, diffing the panel render, calibrating the `config/profiles.rs` weight table, refining `AssetEvaluator` overrides (`ew_compression_factor`, `sl_extension_factor`, `tp_compression_factor`, `score_adjustments`), and refreshing fixtures. Plus 3 cross-asset cleanup commits (X1 trait test coverage, X2 weight-table doc, X3 exec-summary refresh). Total scope ~30-40h. **1.7.18.A Altcoin V62** (~6-8h): `altProfileMult` token-class resolution, `altClean`/`altChaos`/`altFakeImpulse` candle-quality signals, `altMaxChaseATR` no-chase TP gate. **1.7.18.B BTC V61.9** (~5-7h): Pine `failReason` priority-text port, V61.6 deep-add `longKill`/`shortKill` hard-block, V61.7 consensus edge panel surface. **1.7.18.C Forex V58** (~4-6h): port `longScoreRaw`/`shortScoreRaw` to match Pine V58 exact weights, `htfBlockLong`/`htfBlockShort` hard gate, Forex-specific penalty stack (`rollover_avoid`, `atr_too_high`, `adx_overheated`). **1.7.18.D IDX V5 + StocksUs split** (~7-9h): separate `downScore`/`down_risk_high` panel surface, profile score keys rebalance against Pine V5 lines 295-317, split `StocksUsEvaluator` into its own module with SPY/QQQ benchmark + US sessions, IDX-specific SR-band TP cap. Recommended landing order: A → D → C → B (highest user-visible divergence first, BTC last because the V61.9 port is closest to Pine already). **Drafted 2026-06-09** — implementation deferred. See `docs/phase1_pine_parity_plan.md` § 1.7.18 for the full per-asset commit plan with Pine line citations.
